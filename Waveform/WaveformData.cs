...
namespace KiriMusicHelper.Waveform
{
    public static class WaveformData
    {
        ...
        public static async Task<float[][]> GetData(ISampleSource waveSource)
        {
            if (waveSource == null)
            ...
            {
                //var sampleSource = waveSource;
                var sampleSource = new InterruptDisposeChainSource(waveSource);
                ...
                Length = sampleCount;
                sampleSource.Position = 0;

                return waveformDataChannels.Select(x => x.GetData()).ToArray();
            });
        }

        public class InterruptDisposeChainSource : ISampleAggregator
        {
            private readonly ISampleSource _audioSource;

            public InterruptDisposeChainSource(ISampleSource audioSource)
            {
            ...
            public ISampleSource BaseSource
            {
